    ...
    $('div.share42init').each(function (idx) {
      ...
      function fb_count(url) {
        ...
        $.getJSON('http://graph.facebook.com/?callback=?&ids=' + url, function (data) {
          ...
          if (shares > 0 || z == 1) el.find('a[data-count="fb"]').after('<span class="b-socialButtons__counter">' + shares + '</span>')
        })
      }
      fb_count(u);
      ...
      function odkl_count(url) {
        ...
        window.ODKL.updateCount = function (idx, number) {
          shares = number;
          if (shares > 0 || z == 1) $('div.share42init').eq(idx).find('a[data-count="odkl"]').after('<span class="b-socialButtons__counter">' + shares + '</span>')
        }
      }
      odkl_count(u);
      ...
      function twi_count(url) {
        ...
        $.getJSON('http://urls.api.twitter.com/1/urls/count.json?callback=?&url=' + url, function (data) {
          shares = data.count;
          if (shares > 0 || z == 1) el.find('a[data-count="twi"]').after('<span class="b-socialButtons__counter">' + shares + '</span>')
        })
      }
      twi_count(u);
      ...
      function vk_count(url) {
        ...
        window.VK.Share = {
          count: function (idx, number) {
            shares = number;
            if (shares > 0 || z == 1) $('div.share42init').eq(idx).find('a[data-count="vk"]').after('<span class="b-socialButtons__counter">' + shares + '</span>')
          }
        }
      }
      vk_count(u);
      ...
      var l = '';
      for (j = 0; j < s.length; j++) l += '<li class="b-socialButtons__item"><a class="b-socialButtons__link" rel="nofollow" style="background:url(' + f + 'icons.png) -' + 26 * j + 'px 0 no-repeat" href=' + s[j] + ' target="_blank"></a></li>';
      el.html('<ul class="b-socialButtons__list">'+l+'</ul>');
    })
  })
})(jQuery);